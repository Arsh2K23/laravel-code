<template>
  <div class="space-y-6">
    ...
    <div class="bg-white shadow rounded-lg p-6">
      ...
      <p class="text-gray-600 mt-1">Manage all tenants, restaurants, and warehouses</p>
    </div>
      ...
      <div class="bg-white overflow-hidden shadow rounded-lg">
        <div class="p-5">
          <div class="flex items-center">
            ...
            <div class="ml-5 w-0 flex-1">
              <dl>
                <dt class="text-sm font-medium text-gray-500 truncate">Active Restaurants</dt>
                <dd class="text-lg font-medium text-gray-900">{{ stats.restaurants || 0 }}</dd>
              </dl>
            </div>
          </div>
        </div>
      ...
      <div class="bg-white overflow-hidden shadow rounded-lg">
        <div class="p-5">
          <div class="flex items-center">
            ...
            <div class="ml-5 w-0 flex-1">
              <dl>
                <dt class="text-sm font-medium text-gray-500 truncate">Active Warehouses</dt>
                <dd class="text-lg font-medium text-gray-900">{{ stats.warehouses || 0 }}</dd>
              </dl>
            </div>
          </div>
        </div>
          ...
          <div class="flex items-center">
            ...
            <div class="ml-5 w-0 flex-1">
              <dl>
                ...
                <dd class="text-lg font-medium text-gray-900">{{ stats.recent_tenants || 0 }}</dd>
              </dl>
            </div>
          </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="bg-white shadow rounded-lg">
        ...
        <div class="p-6">
          <div v-if="recentTenants.length === 0" class="text-center text-gray-500 py-8">
            No recent tenants
          </div>
    ...
    <div class="bg-white shadow rounded-lg p-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <span class="text-sm font-medium text-indigo-900">Add Tenant</span>
        </router-link>
        ...
        >
          ...
          <span class="text-sm font-medium text-blue-900">Settings</span>
        </router-link>
        ...
        <button class="flex items-center p-4 bg-purple-50 rounded-lg hover:bg-purple-100 transition-colors">
          ...
          <span class="text-sm font-medium text-purple-900">Backup</span>
        </button>
      </div>
    </div>
  </div>
    ...
    const fetchDashboardData = async () => {
      try {
        const [statsResponse, tenantsResponse] = await Promise.all([
          ...
          axios.get('/api/super-admin/tenants?per_page=5')
        ])

        stats.value = statsResponse.data
        recentTenants.value = tenantsResponse.data.data
      } catch (error) {
